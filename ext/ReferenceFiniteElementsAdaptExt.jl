...
function Adapt.adapt_structure(to, re::ReferenceFE)
  ...
  surface_interpolants = Adapt.adapt_structure(to, re.surface_interpolants)

  return ReferenceFE{
    # Itype, N, D, Ftype, N * D, N * D * D, Q, 
    Itype, Ftype, N, D, Q,
    typeof(ref_fe_type), typeof(interpolants), typeof(surface_interpolants), typeof(nodal_coordinates), 
    typeof(edge_nodes), typeof(face_nodes), typeof(interior_nodes)
  }(ref_fe_type, nodal_coordinates, edge_nodes, face_nodes, interior_nodes, interpolants, surface_interpolants)
end
