...
function shape_function_value(e::Quad{Lagrange, PD}, _, ξ) where PD
    coords_x = dof_coordinates(boundary_element(e))
    coords_y = dof_coordinates(boundary_element(e))
    N_x = shape_function_value(boundary_element(e), coords_x, ξ[1])
    N_y = shape_function_value(boundary_element(e), coords_y, ξ[2])
...
function shape_function_gradient(e::Quad{Lagrange, PD}, X, ξ) where PD
    ...
    N_x = shape_function_value(boundary_element(e), coords_x, ξ[1])
    N_y = shape_function_value(boundary_element(e), coords_y, ξ[2])
    ∇N_x = shape_function_gradient(boundary_element(e), coords_x, ξ[1])
    ∇N_y = shape_function_gradient(boundary_element(e), coords_y, ξ[2])
  
    # return N_x * N_y
  
    ∇N = Matrix{eltype(ξ)}(undef, 2, num_cell_dofs(e))
    ...
    offset = 4
    for n in 2:PD
        ∇N[1, offset + n - 1] = ∇N_x[n + 1] * N_y[1]
        ∇N[2, offset + n - 1] = N_x[n + 1] * ∇N_y[1]
    end

    # facet 2
    offset += PD - 1
    for n in 2:PD
        ∇N[1, offset + n - 1] = ∇N_x[2] * N_y[n + 1]
        ∇N[2, offset + n - 1] = N_x[2] * ∇N_y[n + 1]
    end

    # facet 3
    offset += PD - 1
    for n in 2:PD
        ∇N[1, offset + n - 1] = ∇N_x[n + 1] * N_y[2]
        ∇N[2, offset + n - 1] = N_x[n + 1] * ∇N_y[2]
    end

    # facet 4
    offset += PD - 1
    for n in 2:PD
        ∇N[1, offset + n - 1] = ∇N_x[1] * N_y[n + 1]
        ∇N[2, offset + n - 1] = N_x[1] * ∇N_y[n + 1]
    end
...
function shape_function_hessian(e::Quad{Lagrange, PD}, X, ξ) where PD
    ...
    ∇N_x = shape_function_gradient(boundary_element(e), coords_x, ξ[1])
    ∇N_y = shape_function_gradient(boundary_element(e), coords_y, ξ[2])
    ∇∇N_x = shape_function_hessian(boundary_element(e), coords_x, ξ[1])
    ∇∇N_y = shape_function_hessian(boundary_element(e), coords_y, ξ[2])
  
    ∇∇N = Array{eltype(ξ), 3}(undef, 2, 2, num_cell_dofs(e))
    ...
    offset = 4
    for n in 2:PD
        ∇∇N[1, 1, offset + n - 1] = ∇∇N_x[n + 1] * N_y[1]
        ∇∇N[1, 2, offset + n - 1] = ∇N_x[n + 1] * ∇N_y[1]
        ∇∇N[2, 1, offset + n - 1] = ∇N_x[n + 1] * ∇N_y[1]
        ∇∇N[2, 2, offset + n - 1] = N_x[n + 1] * ∇∇N_y[1]
    end

    # facet 2
    offset += PD - 1
    for n in 2:PD
        ∇∇N[1, 1, offset + n - 1] = ∇∇N_x[2] * N_y[n + 1]
        ∇∇N[1, 2, offset + n - 1] = ∇N_x[2] * ∇N_y[n + 1]
        ∇∇N[2, 1, offset + n - 1] = ∇N_x[2] * ∇N_y[n + 1]
        ∇∇N[2, 2, offset + n - 1] = N_x[2] * ∇∇N_y[n + 1]
    end

    # facet 3
    offset += PD - 1
    for n in 2:PD
        ∇∇N[1, 1, offset + n - 1] = ∇∇N_x[n + 1] * N_y[2]
        ∇∇N[1, 2, offset + n - 1] = ∇N_x[n + 1] * ∇N_y[2]
        ∇∇N[2, 1, offset + n - 1] = ∇N_x[n + 1] * ∇N_y[2]
        ∇∇N[2, 2, offset + n - 1] = N_x[n + 1] * ∇∇N_y[2]
    end

    # facet 4
    offset += PD - 1
    for n in 2:PD
        ∇∇N[1, 1, offset + n - 1] = ∇∇N_x[1] * N_y[n + 1]
        ∇∇N[1, 2, offset + n - 1] = ∇N_x[1] * ∇N_y[n + 1]
        ∇∇N[2, 1, offset + n - 1] = ∇N_x[1] * ∇N_y[n + 1]
        ∇∇N[2, 2, offset + n - 1] = N_x[1] * ∇∇N_y[n + 1]
    end
