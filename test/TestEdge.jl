...
#       # e = ReferenceFiniteElements.element_stencil(Edge(), degree)
#       # @test e.degree == degree
#       # @test all(X -> X >= 0. && X <= 1., e.coordinates)
#       e = ReferenceFE(Edge(), degree)
#       @test all(X -> X >= 0. && X <= 1., e.nodal_coordinates)
#     end
#   end
...
#       @test e.nodal_coordinates[v_nodes[1]] ≈ 0.
#       @test e.nodal_coordinates[v_nodes[2]] ≈ 1.

#       if length(e.interior_nodes) > 0
#         @test all(x -> x > 0. && x < 1., e.nodal_coordinates[e.interior_nodes])
#       end
#     end
#   end
# end
...
#       _, ws = ReferenceFiniteElements.quadrature_points_and_weights(Edge(), degree)
#       @test all(w -> w > 0., ws)
#     end
#   end
