@testset ExtendedTestSet "ElementStencils.jl - Tri3 implementation" begin
  @testset "Test Tri3 element interpolant points in element" begin
    for q_degree in [1, 2, 3, 4, 5, 6]
      e = ElementStencil(Tri3(), q_degree)
  ...
  #     e_2d = ElementStencil(Tri3(), q_degree)
  ...
  @testset "Test Tri3 grad interpolation" begin
    ...
    e = ElementStencil(Tri3(), q_degree)
    ∇N_ξ = ReferenceFiniteElements.shape_function_gradients(Tri3(), x)
    fn = polyval2d.(e.coordinates[1, :], e.coordinates[2, :], (poly_coeffs,))
