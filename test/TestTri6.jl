@testset ExtendedTestSet "ReferenceFEStencils.jl - Tri6 implementation" begin
  @testset "Test Tri6 element interpolant points in element" begin
    for q_degree in [1, 2, 3, 4, 5, 6]
      e = ReferenceFEStencil(Tri6(), q_degree)
  ...
  @testset "Test Tri6 interpolation" begin
    ...
    e = ReferenceFEStencil(Tri6(), q_degree)
    Ns = ReferenceFiniteElements.shape_function_values_int(Tri6(), x)
  ...
  @testset "Test Tri6 grad interpolation" begin
    ...
    e = ReferenceFEStencil(Tri6(), q_degree)
    ∇N_ξ = ReferenceFiniteElements.shape_function_gradients_int(Tri6(), x)
    fn = polyval2d.(e.coordinates[1, :], e.coordinates[2, :], (poly_coeffs,))
